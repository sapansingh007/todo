...
// Hugging Face processing (client-side trigger to server proxy)
async function hfProcess() {
    const text = ocrTextArea ? ocrTextArea.value : '';
    extractResult.innerHTML = '';
    if (!text) { extractResult.textContent = 'No OCR text to send.'; return; }
    extractLoading.style.display = 'inline';
    try {
        const res = await fetch('/hf-process', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text }) });
        if (res.status === 501) {
            const j = await res.json();
            extractResult.textContent = 'Server not configured for Hugging Face: ' + (j && j.error ? j.error : 'no token');
            return;
        }
        if (!res.ok) {
            const t = await res.text();
            extractResult.textContent = 'HF processing failed: ' + (t || res.statusText);
            return;
        }
        const j = await res.json();
        ...
        extractResult.appendChild(pre);
    } catch (e) {
        extractResult.textContent = 'HF request failed: ' + (e && e.message ? e.message : '');
    } finally {
        extractLoading.style.display = 'none';
    }
}

hfBtn && hfBtn.addEventListener('click', hfProcess);
